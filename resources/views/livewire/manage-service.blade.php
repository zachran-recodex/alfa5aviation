<div class="bg-white sm:rounded-lg p-6 relative rounded-xl border border-neutral-200 dark:border-neutral-700">
    ...
    <div>
        <div class="mb-4">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="w-64">
                    <flux:input wire:model.debounce.300ms="search" icon="magnifying-glass" placeholder="Cari service..." />
                </div>

                <flux:button icon="plus" class="cursor-pointer bg-primary-600! hover:bg-primary-700! text-white!" wire:click="create">Add Service</flux:button>
            </div>
        </div>
                ...
                @forelse ($services as $service)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <flux:button class="cursor-pointer bg-yellow-400! hover:bg-yellow-500! text-white!" wire:click="edit({{ $service->id }})" icon="pencil"></flux:button>
                            <flux:button class="cursor-pointer bg-red-500! hover:bg-red-600! text-white!" wire:click="confirmDelete({{ $service->id }})" icon="trash"></flux:button>
                        </td>
                    </tr>
                @empty
                    <tr>
    ...
    <flux:modal name="form" class="w-7xl" @close="resetForm">
        <div class="space-y-6">
            <div>
                <flux:heading size="lg">{{ $isEditing ? 'Edit Service' : 'Add Service' }}</flux:heading>
            </div>
            ...
            <form wire:submit.prevent="save" class="space-y-6">
                ...
                @if ($image)
                    <div>
                        ...
                        <div class="mt-2">
                            <img src="{{ Storage::url($image) }}" alt="Service Image" class="h-32 w-auto object-cover rounded">
                        </div>
                    </div>
                @endif
