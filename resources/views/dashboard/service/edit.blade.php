    ...
    <div class="w-full mx-auto my-4">
        <div class="border border-gray-200 rounded-lg bg-gray-50 shadow-sm">
            ...
            <div class="p-4">
                ...
                <form action="{{ route('dashboard.service.update', $service) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-12 gap-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-span-6">
                        ...
                        <img src="{{ asset('storage/' . $service->image) }}" alt="{{ $service->title }}" class="border rounded-lg w-full h-64 object-cover">
                    </div>

                    <div class="col-span-12">
                        <label for="description" class="form-label">Description</label>
                        <textarea id="description" name="description" class="form-input" rows="5">{{ old('description', $service->description) }}</textarea>
                        @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-span-12 flex items-center justify-between">
                        <label class="inline-flex items-center cursor-pointer">
                            ...
                            <input type="checkbox" id="is_active" name="is_active" value="1" class="sr-only peer" {{ old('is_active', $service->is_active) ? 'checked' : '' }}>
